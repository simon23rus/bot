...
def bot_start_cook(bot, update, menu, order):
...
def bot_alarm(time, bot, update, menu, order):
    def alarm(bot, update, menu, order):
...
def bot_end_order(bot, update, menu, order):
    bot.sendMessage(update.message.from_user.id, 'Спасибо за ваш заказ!')
    return 'return'

def bot_blacklist(time, bot, update, menu, order):
    def blacklist(bot, update, menu, order):
        status = wait_end_order(update.message.from_user.id, time, order)
        if status == True:
            return 'end order'
...
def bot_banned(bot, update, menu, order):
    return 'unbanned'
